...
# hsa00001.keg
# https://www.kegg.jp/brite/hsa00001.keg
#################################################################################################
...
#   !     #     %     +     A     B     C     D 
#   2    11     1     1     8   112   534 50200 
table(substr(line,1,1))
#  5  3176  4799  9682 12326 20533 33647 50252
...
# 只提取前6大类A
line = line[1:(which(substr(line,1,1)=="A")[7]-1)]
...
# 提取通路编号
temp = str_split(kegg_gene_2,"[:\\]]",simplify = T)
# 部分通路没有 hsa***** 字样的编号,再次提取指定编号
ID = which(temp[,2]=="")
temp[ID,2] = str_split(temp[ID,1]," ",simplify = T)[,5]
# 提取C编号和对应的通路名
kegg_gene_2 = cbind(temp[,2],substr(kegg_gene_2,12,99999))
# 清除通路名中多余的通路编号
